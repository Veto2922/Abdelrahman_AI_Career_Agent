...
### [Arabic-English Translation Transformer](https://github.com/Veto2922/transformer-arabic-english-translation/tree/main)
Engineered and implemented a 72M-parameter Transformer model from scratch in PyTorch for Arabic-to-English machine translation. This end-to-end project involved designing a custom data pipeline with specialized preprocessing for Arabic's morphological complexity, building all core architectural components (e.g., multi-head attention, positional encodings), and integrating modern training optimizations.

#### Achievements:
* Architectural Implementation and Modernization: Successfully built a complete Transformer architecture by manually implementing all foundational components in PyTorch, including embeddings, attention mechanisms, and the encoder-decoder stack. Enhanced the vanilla architecture with modern techniques such as learned positional embeddings, pre-layer normalization, label smoothing, and mixed-precision (FP16) training to improve stability and accelerate convergence.
* End-to-End MLOps Pipeline: Developed a comprehensive workflow from data ingestion to deployment, including creating a custom data loader, training custom SentencePiece tokenizers, and validating the model with greedy and beam search decoding (achieving a BLEU score of 0.237 after three epochs).
